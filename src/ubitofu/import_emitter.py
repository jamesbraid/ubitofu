...
def slugify(name: str) -> str:
    slug = re.sub(r"[^a-z0-9]+", "_", name.strip().lower()).strip("_")  # pragma: no mutate — equivalent: alternate strip() arg mutations coincide on lowercased alnum slugs; strip behaviour is covered by test_slugify_strips_edge_underscores
...
def assign_slugs(
    ...
    for t in targets:
        ...
        seen[key] = seen.get(key, 0) + 1  # pragma: no mutate — equivalent: seen is a per-base suffix cache; get(None,0) still yields correct slugs because the `used`/`reserved` while-loop reconstructs them (verified by test_assign_slugs_used_collision_forces_distinct), differing only in speed
        n = seen[key]
